...
#define TAG "de.appwerft.audionotification"

using namespace v8;

static Persistent<Object> bindingCache;

static void Tiaudionotification_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Tiaudionotification.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TiaudionotificationBindings::lookupGeneratedInit(
...
static void Tiaudionotification_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Tiaudionotification_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Tiaudionotification_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TiaudionotificationBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiaudionotificationBinding = {
	"de.appwerft.audionotification",
	Tiaudionotification_init,
	Tiaudionotification_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_de_appwerft_audionotification_TiaudionotificationBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.audionotification", &TiaudionotificationBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TiaudionotificationBindings::lookupGeneratedInit));
}
