...
void NotificationProxy::remove(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "remove()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(NotificationProxy::javaClass, "remove", "()V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'remove' with signature '()V'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
